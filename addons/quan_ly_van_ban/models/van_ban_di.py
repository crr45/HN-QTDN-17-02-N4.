...
class VanBanDi(models.Model):
    ...
    ai_suggested_employee_id = fields.Many2one('nhan_vien', string="Người ký đề xuất (AI)", readonly=True)
    ...
    def action_ai_analyze(self):
        for record in self:
            try:
                ...
                # Tìm nhân viên tương ứng (Người ký)
                if res.get('employee'):
    ...
    def action_apply_ai_suggestions(self):
        for record in self:
            ...
            if record.ai_suggested_employee_id:
                vals['nguoi_ky_id'] = record.ai_suggested_employee_id.id
            if vals:
                record.write(vals)
                return {
                    'type': 'ir.actions.client',
